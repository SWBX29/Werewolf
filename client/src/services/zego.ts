...
import type {
  ...
  ZegoPublishStateEvent,
  ZegoPlayStateEvent,
  ZegoStreamQualityEvent,
...
export class ZegoVoiceService {
  ...
  async loginRoom(roomID: string, userID: string, userName: string): Promise<boolean> {
    ...
    try {
      ...
      // 7. 默认静音麦克风（与 store 的 isMicrophoneMuted: true 保持一致）
      // 调用方（如 NightPhase、LobbyPanel）会根据场景决定是否取消静音
      this._zg.muteMicrophone(true);
      this._isMicrophoneMuted = true;
  ...
  private _registerEventHandlers(): void {
    ...
    this._zg.on('roomStateChanged', (roomID: string, reason: string, errorCode: number, extendedData: string) => {
      ...
      // 兼容处理：Zego SDK 可能传入 PascalCase（Logined）或 UPPERCASE（LOGINED）格式
      const normalizedReason = reason.toUpperCase();

      switch (normalizedReason) {
        case 'LOGINING':
          this._connectionState = 'CONNECTING';
          console.log(`[${timestamp}][ZegoVoice] 房间登录中: roomID=${roomID}`);
          break;
        case 'LOGINED':
          this._connectionState = 'CONNECTED';
        ...
        case 'LOGOUT_FAILED':
          ...
          break;
        default:
          console.warn(`[${timestamp}][ZegoVoice] 未知的房间状态变化原因: reason=${reason}, errorCode=${errorCode}`);
          break;
      }

      if (this._callbacks.onRoomStateChanged) {
        const event: ZegoRoomStateEvent = { roomID, reason, errorCode, extendedData };
    ...
    // 推流状态变化回调
    this._zg.on('publisherStateUpdate', (result: { streamID: string; state: string; errorCode: number; extendedData: string }) => {
      const timestamp = new Date().toISOString().substring(11, 23);
      console.log(`[${timestamp}][ZegoVoice] publisherStateUpdate: streamID=${result.streamID}, state=${result.state}, errorCode=${result.errorCode}`);

      if (this._callbacks.onPublisherStateUpdate) {
        const event: ZegoPublishStateEvent = {
          streamID: result.streamID,
          state: result.state,
          errorCode: result.errorCode,
          extendedData: result.extendedData,
        };
        this._callbacks.onPublisherStateUpdate(event);
      }
    });

    // 拉流状态变化回调
    this._zg.on('playerStateUpdate', (result: { streamID: string; state: string; errorCode: number; extendedData: string }) => {
      const timestamp = new Date().toISOString().substring(11, 23);
      console.log(`[${timestamp}][ZegoVoice] playerStateUpdate: streamID=${result.streamID}, state=${result.state}, errorCode=${result.errorCode}`);

      if (this._callbacks.onPlayerStateUpdate) {
        const event: ZegoPlayStateEvent = {
          streamID: result.streamID,
          state: result.state,
          errorCode: result.errorCode,
          extendedData: result.extendedData,
        };
        this._callbacks.onPlayerStateUpdate(event);
      }
    });

    // 推流质量回调（每3秒触发一次）
    this._zg.on('publishQualityUpdate', (streamID: string, stats: { video: { videoQuality: number }; audio: { audioQuality: number } }) => {
      if (this._callbacks.onPublishQualityUpdate) {
        const event: ZegoStreamQualityEvent = {
          streamID,
          userID: this._streamUserMap.get(streamID) ?? this._parseUserIDFromStreamID(streamID),
          videoQuality: _mapQualityGrade(stats.video?.videoQuality ?? -1),
          audioQuality: _mapQualityGrade(stats.audio?.audioQuality ?? -1),
        };
        this._callbacks.onPublishQualityUpdate(event);
      }
    });

    // 拉流质量回调（每3秒触发一次）
    this._zg.on('playQualityUpdate', (streamID: string, stats: { video: { videoQuality: number }; audio: { audioQuality: number } }) => {
      if (this._callbacks.onPlayQualityUpdate) {
        const event: ZegoStreamQualityEvent = {
          streamID,
          userID: this._streamUserMap.get(streamID) ?? this._parseUserIDFromStreamID(streamID),
          videoQuality: _mapQualityGrade(stats.video?.videoQuality ?? -1),
        ...
        this._callbacks.onPlayQualityUpdate(event);
      }
    });
  }
