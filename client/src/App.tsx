...
async function preloadGameComponents(
  ...
  try {
    ...
    // 4. 预加载 ZEGO SDK（最低优先级，体积最大）— 仅在启用语音时
    if (useGameStore.getState().enableVoice) {
      try {
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // 仅当启用语音且切换到游戏视图时才初始化 Zego SDK
    if (enableVoice && currentView === 'game' && !zegoInitPromise.current && !zegoInitialized.current) {
  ...
  useEffect(() => {
    ...
    // 进入游戏视图且有房间信息且启用语音时，加入语音房间
    if (enableVoice && currentView === 'game' && roomCode && playerId && nickname) {
      // 避免重复加入同一房间
    ...
    // 语音禁用、房间解散或离开游戏视图时，退出语音房间
    if (!enableVoice || currentView !== 'game' || roomDissolvedData || !roomCode) {
