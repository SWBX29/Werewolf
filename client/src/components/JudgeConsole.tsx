...
const JudgeConsole: React.FC = () => {
  ...
  // 夜间确认弹窗
  const [nightConfirmAction, setNightConfirmAction] = useState<(() => void) | null>(null);
  ...
  // 编辑中的发言顺序
  const [editingSpeechOrder, setEditingSpeechOrder] = useState<number[] | null>(null);
  // 跳过发言操作锁定（防止快速点击）
  ...
  // 夜间阶段语音操作确认包装
  const isNightPhase = state.phase === 'NIGHT' || state.phase === 'NIGHT_SETTLEMENT' || state.phase === 'PRE_NIGHT';
  const withNightConfirm = useCallback((action: () => void) => {
    if (isNightPhase) {
      ...
      <div className="card mb-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <span className={`tag ${state.phase.includes('NIGHT') ? 'bg-indigo-900 text-indigo-300' : 'bg-amber-900 text-amber-300'}`}>
            ...
            {state.phase === 'DAY_SPEECH' && state.config.firstDayDoubleSpeech && state.round === 1 && state.currentSpeechRound === 2 && ' (第2轮)'}
          </span>
        ...
        <div className="space-y-4 lg:col-span-1">
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-3">
              ...
              {editingSpeechOrder ? (
                <div className="flex gap-2">
                  <button onClick={confirmSpeechOrder} className="btn-primary text-xs">
                    确认修改
                  </button>
                  <button onClick={cancelSpeechOrderEdit} className="btn-secondary text-xs">
                    取消
                  </button>
                </div>
              ) : (
                <button
                  onClick={() => setEditingSpeechOrder([...state.speechOrder])}
                  className="btn-secondary text-xs"
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      {editingSpeechOrder && index < currentSpeechOrder.length - 1 && (
                        <button
                          onClick={() => moveSpeechOrder(index, index + 1)}
                          className="text-gray-500 hover:text-gray-300 text-xs"
                        >
                    ...
                    {!editingSpeechOrder && state.phase === 'DAY_SPEECH' && (
                      <button
                        ...
                        disabled={isSkippingSpeech || seatNumber !== (state.speechOrder[state.currentSpeakerIndex] ?? null)}
                      >
                ...
                {Object.entries(state.nightActions).map(([roleId, action]: [string, NightActionData]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Bug 修复：女巫使用毒药时 targetSeat 为 null，但 extra.usePoison 为 true
                          应检查是否真的未使用任何药水，而非仅依赖 targetSeat */}
                      {action.submitted && roleId === 'witch' && action.targetSeat === null && !(extra.useAntidote || extra.usePoison) && (
                        <div className="mt-1 text-xs text-gray-500">未使用药水</div>
                      )}
                      {/* 女巫使用毒药时显示毒药目标 */}
                      {action.submitted && roleId === 'witch' && extra.usePoison && extra.poisonTarget != null && (
                        <div className="mt-1 text-xs text-gray-400">
                          → 毒药目标：<span className="text-purple-400">{extra.poisonTarget}号 {state.players.find(p => p.seatNumber === extra.poisonTarget)?.nickname ?? ''}</span>
                        </div>
                      )}
                    </div>
                  );
        ...
        <div className="card mt-4">
          ...
          {connectionState === 'CONNECTED' ? (
                  ...
                  return (
                    <div key={player.id} className="flex flex-col items-center gap-1">
                      <div className={seatClass}>
                        <span className="text-xs font-bold text-gray-400">{player.seatNumber}号</span>
                      ...
                      <button
                        onClick={() => {
                          withNightConfirm(() => {
                            if (isMuted) {
                              getZegoVoiceService().unmuteRemoteAudioByUserID(player.id);
                ...
                <button
                  onClick={() => {
                    withNightConfirm(() => {
                      ...
                      setMutedPlayerIds(new Set(nonJudgePlayers.filter((p) => p.status === 'alive').map((p) => p.id)));
            ...
            <div className="text-center py-4">
              <p className="text-sm text-gray-600 mb-2">语音未连接，请检查服务是否启动</p>
              {connectionState === 'DISCONNECTED' && (
                <button
                  onClick={() => manualReconnect()}
                  disabled={isManualReconnecting}
