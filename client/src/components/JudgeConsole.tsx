...
const JudgeConsole: React.FC = () => {
  ...
  const [explodeTargetSeat, setExplodeTargetSeat] = useState<number | null>(
    null,
  );
  const [showDissolveConfirm, setShowDissolveConfirm] = useState(false);
  const [voiceActionFeedback, setVoiceActionFeedback] = useState<string | null>(
    null,
  );
  // 法官语音控制：被静音的玩家 ID 集合
  const [mutedPlayerIds, setMutedPlayerIds] = useState<Set<string>>(new Set());
  // 夜间确认弹窗
  const [nightConfirmAction, setNightConfirmAction] = useState<
    (() => void) | null
  >(null);
  ...
  // 编辑中的夜间顺序
  const [editingNightOrder, setEditingNightOrder] = useState<RoleId[] | null>(
    null,
  );
  // 编辑中的发言顺序
  const [editingSpeechOrder, setEditingSpeechOrder] = useState<number[] | null>(
    null,
  );
  // 跳过发言操作锁定（防止快速点击）
  const [isSkippingSpeech, setIsSkippingSpeech] = useState(false);
                ...
                {players
                  ...
                  .map((p) => (
                    ...
                    >
                      ...
                      {p.role && (
                        <span
                          className={`tag ${isEvilRole(p.role as RoleId) ? "tag-evil" : "tag-good"}`}
                        >
  ...
  // ---- 发言顺序编辑 ----
  const currentSpeechOrder = editingSpeechOrder ?? state.speechOrder;

  const moveSpeechOrder = (fromIndex: number, toIndex: number) => {
    const order = [...currentSpeechOrder];
    const [moved] = order.splice(fromIndex, 1);
    order.splice(toIndex, 0, moved);
    setEditingSpeechOrder(order);
  };

  const confirmSpeechOrder = () => {
    if (editingSpeechOrder) {
      modifySpeechOrder(editingSpeechOrder);
      setEditingSpeechOrder(null);
    }
  };

  const cancelSpeechOrderEdit = () => {
    setEditingSpeechOrder(null);
  };
  ...
  const isNightPhase =
    state.phase === "NIGHT" ||
    state.phase === "NIGHT_SETTLEMENT" ||
    state.phase === "PRE_NIGHT";
  const withNightConfirm = useCallback(
    (action: () => void) => {
      if (isNightPhase) {
        setNightConfirmAction(() => action);
      } else {
        action();
      }
    },
    ...
    <div className="min-h-screen p-4 max-w-6xl mx-auto">
      ...
      <div className="card mb-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-gray-400">
            房间码：
            <span className="font-mono text-white">{state.roomCode}</span>
          </span>
          <span className="text-sm text-gray-400">
            第 <span className="text-white">{state.round}</span> 轮
          </span>
          <span
            className={`tag ${state.phase.includes("NIGHT") ? "bg-indigo-900 text-indigo-300" : "bg-amber-900 text-amber-300"}`}
          >
            {PHASE_NAMES[state.phase]}
            {state.phase === "DAY_SPEECH" &&
              state.config.firstDayDoubleSpeech &&
              state.round === 1 &&
              state.currentSpeechRound === 2 &&
              " (第2轮)"}
          </span>
          {state.isPaused && (
            <span className="tag bg-yellow-900 text-yellow-300">已暂停</span>
          )}
        </div>
        {/* 游戏进行中离开需要确认 */}
        <button
      ...
      {phaseAnnouncement && (
        <div className="card border-wolf-700 mb-4 flex items-center justify-between animate-pulse">
          <span className="text-wolf-300 font-semibold">
            {phaseAnnouncement}
          </span>
          <button
      ...
      {state.phase === "LOBBY" && (
        <div className="flex items-center justify-center">
          <div className="card max-w-lg w-full text-center space-y-6">
            <div>
              <h2 className="text-xl font-bold text-wolf-400 mb-2">
                等待玩家加入
              </h2>
            ...
            <div className="space-y-3">
              <p className="text-sm text-gray-500">
                已准备：
                {state.players.filter((p) => !p.isJudge && p.isReady).length} /{" "}
                {state.players.filter((p) => !p.isJudge).length} 人 （最少{" "}
                {state.config.playerCount} 人）
              </p>
              {(() => {
                const nonJudgePlayers = state.players.filter((p) => !p.isJudge);
                const allReady = nonJudgePlayers.every((p) => p.isReady);
                const enoughPlayers =
                  nonJudgePlayers.length >= state.config.playerCount;
                return (
                  <button
                    onClick={startGame}
              ...
              <p className="text-xs text-gray-600">
                提示：法官为观战者，不参与游戏。请确保人数≥
                {state.config.playerCount}且所有玩家均已准备。
              </p>
            </div>
          </div>
        </div>
      ...
      {state.phase === "GAME_OVER" && gameOverData && (
        <div className="card max-w-lg mx-auto space-y-6 text-center p-6">
          ...
          <p
            className={`text-xl font-bold ${gameOverData.winner === "good" ? "text-blue-400" : "text-red-400"}`}
          >
          ...
          <div className="flex gap-2">
            <button className="btn-primary flex-1" onClick={leaveRoom}>
              返回大厅
            </button>
            <button className="btn-secondary flex-1" onClick={dissolveRoom}>
              解散房间
            </button>
          </div>
        </div>
      ...
      {state.phase !== "LOBBY" && state.phase !== "GAME_OVER" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      {player.isMuted && (
                        <span className="icon-muted text-xs">🔇</span>
                      )}
                      {player.isNightmared && (
                    ...
                    <div className="flex items-center gap-2">
                      {state.phase !== "LOBBY" && (
                        <span
                          className={
                            isEvilRole(player.role) ? "tag-evil" : "tag-good"
                          }
                        >
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-3">
                ...
                {editingNightOrder ? (
                  ...
                  <button
                    onClick={() =>
                      setEditingNightOrder([...state.config.nightActionOrder])
                    }
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        {editingNightOrder &&
                          index < currentNightOrder.length - 1 && (
                            <button
                              onClick={() => moveNightAction(index, index + 1)}
                              className="text-gray-500 hover:text-gray-300 text-xs"
                            >
            ...
            <div className="card">
              ...
              <div className="space-y-2">
                <select
                  ...
                  onChange={(e) =>
                    setOverrideTarget(parseInt(e.target.value) || null)
                  }
                ...
                <select
                  ...
                  onChange={(e) =>
                    setOverrideStatus(e.target.value as PlayerStatus)
                  }
                  className="select-field w-full text-sm"
                  disabled={
                    state.phase === "NIGHT" ||
          ...
          <div className="space-y-4 lg:col-span-1">
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-lg font-semibold">发言顺序</h2>
                {editingSpeechOrder ? (
                  <div className="flex gap-2">
                    <button
                      onClick={confirmSpeechOrder}
                      className="btn-primary text-xs"
                    >
                      确认修改
                    </button>
                    <button
                      onClick={cancelSpeechOrderEdit}
                      className="btn-secondary text-xs"
                    >
                      取消
                    </button>
                  ...
                  <button
                    onClick={() =>
                      setEditingSpeechOrder([...state.speechOrder])
                    }
                    className="btn-secondary text-xs"
                    disabled={state.speechOrder.length === 0}
                  >
                    编辑顺序
                  </button>
                )}
              ...
              <div className="space-y-1">
                ...
                {currentSpeechOrder.map((seatNumber, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        {player.isMuted && (
                          <span className="icon-muted text-xs">🔇禁言</span>
                        )}
                        {editingSpeechOrder &&
                          index < currentSpeechOrder.length - 1 && (
                            <button
                              onClick={() => moveSpeechOrder(index, index + 1)}
                              className="text-gray-500 hover:text-gray-300 text-xs"
                            >
                      ...
                      {!editingSpeechOrder && state.phase === "DAY_SPEECH" && (
                        <button
                          onClick={() => {
                            if (isSkippingSpeech) return;
                            setIsSkippingSpeech(true);
                            skipSpeech(seatNumber);
                            // 500ms后解锁，防止快速点击
                            setTimeout(() => setIsSkippingSpeech(false), 500);
                          }}
                          className="text-xs text-gray-500 hover:text-red-400 disabled:opacity-50 disabled:cursor-not-allowed"
                          disabled={
                            isSkippingSpeech ||
                            seatNumber !==
                              (state.speechOrder[state.currentSpeakerIndex] ??
                                null)
                          }
                        >
                          {isSkippingSpeech ? "处理中..." : "跳过"}
                        </button>
                      )}
                    </div>
                  );
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xl font-bold text-amber-300">
                        {speechTimeRemaining > 0 ? speechTimeRemaining : "--"}
                      </span>
                      <span className="text-xs text-gray-500">秒</span>
                    </div>
                  </div>
                ...
                {voteResult && (
                  <div className="space-y-2">
                    <div className="p-3 rounded bg-night-800/50 border border-night-700">
                      ...
                      <div className="space-y-1">
                        {Object.entries(voteResult.votes).map(
                          ([voterSeat, targetSeat]) => {
                            const voter = state.players.find(
                              (p) => p.seatNumber === Number(voterSeat),
                            );
                            const target = state.players.find(
                              (p) => p.seatNumber === targetSeat,
                            );
                            return (
                              <div
                                key={voterSeat}
                              ...
                              >
                                <span className="text-green-300">
                                  {voterSeat}号 {voter?.nickname ?? ""}
                                </span>
                                {" → "}
                                <span className="text-white">
                                  {targetSeat}号 {target?.nickname ?? ""}
                                </span>
                              </div>
                            );
                          },
                    ...
                    {voteResult.isPK &&
                      voteResult.pkCandidates &&
                      voteResult.pkCandidates.length > 0 && (
                ...
                {Object.keys(state.sheriffElectionVotes).length > 0 && (
                  <div className="space-y-2">
                    <div className="p-3 rounded bg-night-800/50 border border-night-700">
                      ...
                      <div className="space-y-1">
                        {Object.entries(state.sheriffElectionVotes).map(
                          ([voterSeat, targetSeat]) => {
                            const voter = state.players.find(
                              (p) => p.seatNumber === Number(voterSeat),
                            );
                            const target = state.players.find(
                              (p) => p.seatNumber === (targetSeat as number),
                            );
                            return (
                              <div
                                key={voterSeat}
              ...
              <div className="card">
                ...
                <div className="space-y-2">
                  <div className="p-3 rounded bg-night-800/50 border border-night-700">
                    ...
                    <div className="space-y-1">
                      {sheriffTransferRequest.availableTargets.map(
                        (targetSeat) => {
                          const target = state.players.find(
                            (p) => p.seatNumber === targetSeat,
                          );
                  ...
                  <div className="p-3 rounded bg-indigo-950/30 border border-indigo-900">
                    <p className="text-sm text-indigo-400 font-semibold">
                      原警长：{sheriffTransferRequest.deadSheriffSeat}号{" "}
                      {sheriffTransferRequest.deadSheriffNickname}
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  >
                    ...
                    {knightDuelResult.revealedRole && (
                      <p className="text-sm text-gray-400 mt-1">
                        真实身份：
                        {ROLE_META[knightDuelResult.revealedRole]?.name}
                      </p>
                    )}
            ...
            <div className="card">
              ...
              <div className="space-y-2 mb-4">
                ...
                <p className="text-xs text-gray-500">
                  ...
                  {alivePlayers.some(
                    (p) => p.role === "knight" && p.status === "alive",
                  )
                    ? "当前骑士存活，等待骑士主动发动。"
                    : "当前无存活骑士。"}
                </p>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <select
                    ...
                    onChange={(e) =>
                      setExplodeWolfSeat(parseInt(e.target.value) || null)
                    }
                  ...
                  >
                    ...
                    {alivePlayers
                      .filter(
                        (p) =>
                          p.role === "white_wolf_king" && p.status === "alive",
                      )
                      .map((p) => (
                        <option key={p.id} value={p.seatNumber}>
                          {p.seatNumber}号
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setExplodeTargetSeat(parseInt(e.target.value) || null)
                    }
                    className="select-field text-sm"
                  >
                ...
                <button
                  onClick={() => {
                    if (
                      explodeWolfSeat !== null &&
                      explodeTargetSeat !== null
                    ) {
                  ...
                  disabled={
                    explodeWolfSeat === null || explodeTargetSeat === null
                  }
                  className="btn-danger w-full text-sm"
                >
                  触发白狼王自爆
            ...
            {(state.phase === "NIGHT" ||
              ...
              Object.keys(state.nightActions).length > 0) && (
              <div className="card">
                ...
                <div className="space-y-2 mb-3">
                  {Object.entries(state.nightActions).map(
                    ([roleId, action]: [string, NightActionData]) => {
                      const actor = state.players.find(
                        (p) => p.seatNumber === action.actorSeat,
                      );
                      const target =
                        action.targetSeat !== null
                          ? state.players.find(
                              (p) => p.seatNumber === action.targetSeat,
                            )
                          : null;
                      const roleMeta = ROLE_META[roleId as RoleId];
                      const extra = action.extra as Record<
                        string,
                        ...
                        >
                          ...
                          {action.submitted && action.targetSeat !== null && (
                            <div className="mt-1 text-xs text-gray-400">
                              → 目标：
                              <span className="text-white">
                                {action.targetSeat}号 {target?.nickname ?? ""}
                              </span>
                              {/* 角色特定信息 */}
                              {roleId === "seer" &&
                              extra.checkResult != null ? (
                                <span
                                  className={
                                    extra.checkResult === "evil"
                                      ? "text-red-400 ml-1"
                                      : "text-green-400 ml-1"
                                  }
                                >
                              ...
                              {roleId === "witch" &&
                                (extra.useAntidote || extra.usePoison) && (
                                  <span className="ml-1">
                                    {extra.useAntidote ? (
                              ...
                              {roleId === "mechanical_wolf" &&
                                extra.imitateTarget != null && (
                                  <span className="text-cyan-400 ml-1">
                                    模仿 {String(extra.imitateTarget)}号
                                  </span>
                                )}
                            </div>
                          )}
                          ...
                          {action.submitted &&
                            roleId === "witch" &&
                            extra.usePoison &&
                            extra.poisonTarget != null && (
                              <div className="mt-1 text-xs text-gray-400">
                                → 毒药目标：
                                <span className="text-purple-400">
                                  {extra.poisonTarget}号{" "}
                                  {state.players.find(
                                    (p) => p.seatNumber === extra.poisonTarget,
                                  )?.nickname ?? ""}
                                </span>
                              </div>
                            )}
                ...
                {Object.keys(state.wolfVotes).length > 0 && (
                  <div className="mb-3 p-2 rounded bg-red-950/20 border border-red-900/50">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span
                        className={`text-xs ${state.wolfVoteConsensus ? "text-green-400" : "text-yellow-400"}`}
                      >
                    ...
                    <div className="space-y-1 mt-1 text-sm">
                      {state.nightmareTarget !== null && (
                        <p className="text-yellow-400">
                          噩梦恐惧：{state.nightmareTarget}号{" "}
                          {state.players.find(
                            (p) => p.seatNumber === state.nightmareTarget,
                          )?.nickname ?? ""}
                        </p>
                      )}
                      {state.werewolfTarget !== null && (
                        <p>
                          狼人击杀：{state.werewolfTarget}号{" "}
                          {state.players.find(
                            (p) => p.seatNumber === state.werewolfTarget,
                          )?.nickname ?? ""}
                        </p>
                      )}
                      {state.guardProtectTarget !== null && (
                        <p className="text-blue-400">
                          守卫守护：{state.guardProtectTarget}号{" "}
                          {state.players.find(
                            (p) => p.seatNumber === state.guardProtectTarget,
                          )?.nickname ?? ""}
                        </p>
                      )}
                      {state.witchSaveTarget !== null && (
                        <p className="text-green-400">
                          女巫解药：{state.witchSaveTarget}号{" "}
                          {state.players.find(
                            (p) => p.seatNumber === state.witchSaveTarget,
                          )?.nickname ?? ""}
                        </p>
                      )}
                      {state.witchPoisonTarget !== null && (
                        <p className="text-purple-400">
                          女巫毒药：{state.witchPoisonTarget}号{" "}
                          {state.players.find(
                            (p) => p.seatNumber === state.witchPoisonTarget,
                          )?.nickname ?? ""}
                        </p>
                      )}
                    </div>
      ...
      {enableVoice && (
        <div className="card mt-4">
          {/* 连接状态 */}
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold">玩家语音控制台</h2>
          ...
          {/* 操作反馈 */}
          {voiceActionFeedback && (
            <div className="mb-3 p-2 rounded bg-green-950/30 border border-green-900 text-xs text-green-300 text-center">
              {voiceActionFeedback}
            </div>
          )}
            ...
            <>
              ...
              <div className="grid grid-cols-6 gap-2 mb-3">
                {nonJudgePlayers.map((player) => {
                  ...
                  if (isSpeaking && !isDead) seatClass += " seat-speaking";
                  ...
                  return (
                    ...
                    >
                      <div className={seatClass}>
                        ...
                        <span
                          className={`text-sm truncate max-w-full ${isDead ? "text-gray-500" : "text-gray-200"}`}
                        >
                      ...
                      <button
                        onClick={() => {
                          withNightConfirm(() => {
                            if (isMuted) {
                              getZegoVoiceService().unmuteRemoteAudioByUserID(
                                player.id,
                              );
                              ...
                              getZegoVoiceService().muteRemoteAudioByUserID(
                                player.id,
                              );
                              setMutedPlayerIds((prev) => {
                                const next = new Set(prev);
                        ...
                        className={`w-full text-xs py-0.5 px-1 rounded transition-colors ${
                          isMuted
                            ? "bg-red-900/50 hover:bg-red-800/60 text-red-300 border border-red-800/50"
                            : "bg-green-900/50 hover:bg-green-800/60 text-green-300 border border-green-800/50"
                        }`}
                      >
                        {isMuted ? "🔇" : "🎤"}
                      </button>
              ...
              {state.phase === "NIGHT" && (
                <div className="p-2 rounded bg-red-950/30 border border-red-900">
                  ...
                  {state.nightSubPhase && (
                    <div className="mb-2 p-2 rounded bg-indigo-950/30 border border-indigo-900">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => {
                        withNightConfirm(() => {
                          const judgeId = state.players.find(
                            (p) => p.isJudge,
                          )?.id;
                          ...
                          if (currentRole) {
                            const actionPlayerIds = state.players
                              ...
                              .map((p) => p.id);
                            const allowedSpeakers = judgeId
                              ? [judgeId, ...actionPlayerIds]
                              : actionPlayerIds;
                            getZegoVoiceService().setAllowedSpeakers(
                              allowedSpeakers,
                            );
                            setMutedPlayerIds(new Set());
                          ...
                          const wolfIDs = state.players
                            .filter(
                              (p) =>
                                p.status === "alive" &&
                                p.role &&
                                isSharedWolfRole(p.role as RoleId),
                            )
                            .map((p) => p.id);
                  ...
                  <button
                    ...
                    className="btn-secondary text-xs py-1 w-full mt-2"
                  >
                    恢复所有人
                  </button>
                </div>
              ...
              {/* 白天发言模式按钮 */}
              {state.phase === "DAY_SPEECH" && (
                <div className="p-2 rounded bg-amber-950/30 border border-amber-900">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => {
                        const currentSpeakerSeat =
                          state.speechOrder[
                            state.speechOrder.length > 0 ? 0 : -1
                          ];
                        if (
                          ...
                          const speaker = state.players.find(
                            (p) => p.seatNumber === currentSpeakerSeat,
                          );
                          if (speaker) {
                            getZegoVoiceService().setAllowedSpeakers([
                              speaker.id,
      ...
      {nightConfirmAction && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="card max-w-sm w-full mx-4 space-y-4 text-center">
            ...
            <h3 className="text-lg font-bold text-yellow-400">夜晚阶段确认</h3>
            <p className="text-sm text-gray-400">
              当前为夜晚阶段，确认修改玩家发言状态？
            </p>
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => setNightConfirmAction(null)}
                className="btn-secondary"
              >
                取消
              </button>
