/**
 * ============================================================================
 * 服务端入口 — WebSocket 网络层与消息路由
 * ============================================================================
 *
 * 架构说明：
 *   1. 启动 WebSocket 服务器（基于 ws 库）和 HTTP 静态文件服务
 *   2. 读取环境变量（PORT / MONGODB_URI）并连接 MongoDB
 *   3. WebSocket 消息路由与分发，零信任校验所有客户端操作
 *   4. 安全的 DTO 脱敏广播（法官视角 vs 普通玩家视角隔离）
 *   5. 游戏状态快照持久化（支持 tsx watch 热重载后恢复）
 *
 * 设计原则：
 *   - 零信任架构：所有客户端操作在服务端重新校验
 *   - 强制 DTO 过滤：广播时根据接收者身份脱敏
 *   - 法官 payload 包含明文全量数据，普通玩家 payload 禁止包含他人角色
 *   - 预序列化优化：相同消息只 stringify 一次，避免 N 个客户端重复序列化
 * ============================================================================
 */
...
import { connectMongoDB, disconnectMongoDB, isMongoConnected, RoomModel, GameLogModel, WolfChatLogModel } from './models.js';
import { getZegoTokenService } from './services/zegoTokenService.js';
import { connectErrorDb, disconnectErrorDb, logServerError, logClientError, isErrorDbConnected } from './errorLogger.js';
...
function buildPlayerRoomStateDTO(state: RoomState, forPlayerId: string, engine: GameEngine, playerMap: Map<string, Player>): PlayerRoomStateDTO {
  ...
  return {
    ...
    currentSpeechRound: state.currentSpeechRound,
    ...
    enableVoice: state.config.enableVoice,
    ...
    // 待处理的亡语技能：仅技能持有者可见完整信息，其他人仅知有技能待处理
    pendingDeathSkill: state.pendingDeathSkill
      ? (state.pendingDeathSkill.playerId === forPlayerId || player?.isJudge)
        ? state.pendingDeathSkill
        : { ...state.pendingDeathSkill, playerId: '', resumeData: {} }
      : null,
  };
}
...
function handleMessage(ws: WebSocket, rawMessage: string): void {
  ...
  switch (message.type) {
    ...
    case 'REPORT_ERROR':
      logClientError(message.message, message.level, message.stack, message.context, {
        url: client.origin || '',
        userAgent: '',
      });
      break;
    ...
    logServerError(err.message, 'error', {
      messageType: message.type,
      rawMessage: rawMessage.slice(0, 200),
    }, err.stack);
    safeSend(ws, { type: 'ERROR', code: 'INTERNAL_ERROR', message: '消息处理异常' });
  }
}
...
function handleHunterGun(client: ClientContext, message: ClientMessage & { type: 'HUNTER_GUN' }): void {
  ...
  // targetSeat === -1 表示放弃开枪，不需要广播开枪结果
  if (message.targetSeat !== -1) {
    const state = engine.getState();
...
function handleWolfChat(client: ClientContext, message: ClientMessage & { type: 'WOLF_CHAT' }): void {
  ...
  // 构造广播消息（持久化由 LobbyManager.onWolfChat 回调处理）
  const chatMessage: WolfChatMessageType = {
  ...
  const wolfClients = lobby.getRoomClients(client.roomCode).filter((c) => {
    ...
    return p && isSharedWolfRole(p.role, sharedRoles) && p.status === 'alive';
  });
...
async function handleAdminFetchLogs(client: ClientContext, message: ClientMessage & { type: 'ADMIN_FETCH_LOGS' }): Promise<void> {
  ...
  try {
    ...
    if (message.actorSeat !== undefined && message.actorSeat !== null) filter.actorSeat = message.actorSeat;
    if (message.actionTypes && message.actionTypes.length > 0) filter.actionType = { $in: message.actionTypes };
    if (message.phases && message.phases.length > 0) filter.phase = { $in: message.phases };

    const pageSize = message.pageSize || 50;
    const page = message.page || 1;
    const skip = (page - 1) * pageSize;
    const [logs, total] = await Promise.all([
      GameLogModel.find(filter).sort({ timestamp: -1 }).skip(skip).limit(pageSize).lean(),
...
lobby.setPhaseChangeCallback((roomCode: string, phase: GamePhase, subPhase: NightSubPhase | null, round: number) => {
  ...
  // 阶段变化时也广播房间状态，确保客户端获取最新的发言者等信息
  broadcastRoomState(roomCode);
});

// 狼人聊天消息回调
lobby.setWolfChatCallback((roomCode: string, message: WolfChatMessageType) => {
  // 消息已在 handleWolfChat 中直接转发，持久化由 LobbyManager.onWolfChat 处理
});
...
lobby.setGameEventCallback((roomCode: string, eventType: string, data: Record<string, unknown>) => {
  ...
  switch (eventType) {
    ...
    case 'PLAYER_KILLED_BY_GUN':
      broadcastToRoom(roomCode, {
        type: 'PLAYER_KILLED_BY_GUN',
        seatNumber: data.seatNumber as number,
        nickname: data.nickname as string,
        killedBy: data.killedBy as number,
        killedByNickname: data.killedByNickname as string,
...
wss.on('connection', (ws, req) => {
  ...
  const proto = req.headers['x-forwarded-proto'] || (req.headers.origin?.toString().startsWith('https:') ? 'https' : 'http');
  const origin = req.headers.origin?.toString() || (host ? `${proto}://${Array.isArray(host) ? host[0] : host}` : '');
  lobby.registerConnection(ws, origin);
...
async function startServer(): Promise<void> {
  ...
  // 连接错误日志数据库（独立于游戏数据库）
  if (MONGODB_URI) {
    await connectErrorDb(MONGODB_URI);
  }
...
process.on('unhandledRejection', (reason, promise) => {
  ...
  logServerError(String(reason), 'error', { eventType: 'unhandledRejection' }, reason instanceof Error ? reason.stack : undefined);
});

process.on('uncaughtException', (error) => {
  console.error('[Server] 未捕获的异常:', error);
  logServerError(error.message, 'fatal', { eventType: 'uncaughtException' }, error.stack);
  // 对于严重错误，记录后继续运行
