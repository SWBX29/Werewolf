 ...
 *
 * 架构说明：
 *   1. 提供独立的结算逻辑函数，不依赖 GameEngine 实例状态
 *   2. 所有函数接收状态参数，返回结算结果
 *   3. 涵盖胜负判定、死亡连锁、投票结果、发言顺序等核心结算逻辑
 *
 * 设计原则：
 *   - 纯函数：无副作用，便于测试
 ...
 * 检查游戏获胜条件
 *
 * 根据胜负配置（屠边/屠城）判断当前是否有阵营获胜。
 * 隐狼和机械狼始终属于狼人阵营，不受模仿/伪装影响。
 *
 ...
 * 判断守卫守护是否有效
 *
 * 校验规则：首夜可守护自己，不可重复守护同一人。
 *
 * @param guardSeat - 守卫座位号
 * @param targetSeat - 守护目标座位号
 * @param guardProtectedHistory - 守卫历史守护目标列表
 * @param round - 当前轮次
 * @returns 校验结果，包含是否有效及失败原因
 */
export function isGuardProtectionValid(
  ...
  // 不能重复守护同一人（无论隔多少个夜晚）
  if (guardProtectedHistory.includes(targetSeat)) {
 ...
 * 投票结果结算
 *
 * 统计各目标得票数（含警长加权），判断是否需要 PK。
 * 多人同票时进入 PK 阶段。
 *
 * @param votes - 投票记录（投票者座位号 → 目标座位号）
 * @param sheriffSeat - 警长座位号（如果有）
 * @param sheriffVoteWeight - 警长投票权重，默认 1.5
 * @returns 结算结果，包含票数统计、最高票座位、是否 PK 及候选人
 */
 ...
 * 判断机械狼是否可以作为狼人行动
 *
 * 条件：隐狼必须已死亡，且其他 sharedWolfRoles 狼人必须全部死亡。
 * 机械狼仅在所有共同睁眼狼人阵亡后才能代替狼人行动。
 *
 * @param players - 所有玩家列表
 * @param sharedWolfRoles - 共同睁眼的狼人角色列表
 * @returns 是否可以参与狼人投票
 */
export function canMechanicalWolfActAsWolf(
  players: Player[],
  sharedWolfRoles: RoleId[],
): boolean {
  const alivePlayers = players.filter((p) => !p.isJudge && p.status === 'alive');
  const hiddenWolf = alivePlayers.find((p) => p.role === 'hidden_wolf');
  // 隐狼必须已死亡
  if (hiddenWolf) return false;

  // 其他 sharedWolfRoles 狼人必须全部死亡
 ...
 * 判断机械狼模仿是否失败
 *
 * 模仿村民、骑士、白痴时失败，无法获得技能。
 *
 * @param imitatedRole - 模仿的目标角色
 * @returns 是否模仿失败
 */
